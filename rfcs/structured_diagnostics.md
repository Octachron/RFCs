...
The idea of using structured diagnostic is current
...
we can check at runtime when writing the diagnostics if it is valid according to the
scheme for this version.

Adding a version to the type descriptions also makes it easier to enforce a update policy
between version. The policy that I propose is that
...
In the record case, the compiler code will need to keep `deprecated` path to
output the now deleted fields. This means that a record field may progress from
version to version between:
...
However, there are no good options from constructor.
...
Currently, the sum types present in compiler diagnostics are the error kind,
...
the three types for `Format_doc`: one for box kind, one for semantic tags,
and the one for the formatting instructions themselves.
